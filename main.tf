...
resource "azurerm_application_gateway" "ag" {
  ...
  dynamic "probe" {
    for_each = [for app in local.gateways[count.index].app_configuration : {
      ...
      host_name =    "${app.product}-${app.component}.${ app.host_name_suffix_override ? app.host_name_suffix_override : local.gateways[count.index].gateway_configuration.host_name_suffix}"
